...
* **State Machine Based Control:** Utilizes the `smach` library for clear, structured control of the drilling process.
...
| **`ascent_delay_time`** | `double` | `2.0` | Time (s) to wait after collecting sample before ascending. |
...
float64 target\_height\_cm  \# Target height in cm (for autonomous mode)
bool gate\_open            \# True for open, False for closed
bool auger\_on             \# True for on, False for off
bool manual\_up            \# True for manual up, False for off
bool manual\_down          \# True for manual down, False for off
